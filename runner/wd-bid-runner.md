...
- Vol II - Past Performance (.docx): the past-performance content from the GovDash proposal (relevant prior contracts, references, CPARS/PPQs), in its existing format. Reference 02_Clients & Projects in Google Drive only to fill gaps the proposal leaves. STRIP every dollar figure from this volume before saving it - past-performance write-ups routinely list prior contract values, and any price in Vol II breaches the firewall. Remove contract dollar amounts, award values, and rate figures (a plain "$" that is part of prose but carries no number can stay); if a contract's scale must be conveyed, describe it qualitatively (e.g. "multi-year base-plus-options facilities contract") rather than with a dollar amount.
- Vol III - Price / Cost (.docx price schedule + live .xlsx workbook): all pricing - rates, hours, escalation, option years, indirects. This is a pricing workbook built from the Stage 5 pricing - NOT pulled from the GovDash proposal (the proposal carries no price). Keep the workbook editable with formulas intact; follow the Vol III Excel rule below.
